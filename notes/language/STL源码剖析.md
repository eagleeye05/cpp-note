...
不同的编译器**对C++语言的支持程度**不尽相同。作为一个希望具备广泛移植能力的程序库，SGI STL包含了一个**环境组态文件<stl_config.h>，**其中定义了许多==常量==，表示某些==组态的成立与否==，所有STL的头文件都会直接或间接的包含这个组态文件，并以条件式的写法，让预处理器根据各个常量决定取舍哪一段程序的代码，例如：
...
iterator模式定义：提供一种方法，使之能够一次寻访某个容器所含的各个元素，而又无需暴露该容器的内部描述方式
...
**需求**：假设算法中有必要声明一个变量，以“迭代器所指对象的类别”为类型，如何是好？C++不支持`typeof()`，而`typeid()`只能获得类型名称，而不能用来声明变量
...
![image-20200410001123259](../../pics/image-20200410001123259.png)
...
```c++
//声明内嵌类型
temlate <class Iterator>
struct iterator_traits {
    typedef typename Iterator::iterator_cateory iterator_category;
...
```c++
template<class I>
typename iterator_traits<I>::value_type; //函数返回类型
func(I) {
    return *iter;
}
```

![image-20200410104348388](../../pics/image-20200410104348388.png)
...
![image-20200410105138623](../../pics/image-20200410105138623.png)

设计算法时，如果可能，尽量针对上图中的**某种迭代器提供一个明确定义**，并针对更强化的某些迭代器提供另一种定义，这样才能在不同情况下提供最大效率

[以advance()为例](STL源码剖析/advance.md)
...
- 1.设计适当的相应类型，是迭代器的责任
- 2.设计适当的迭代器，是容器的责任
- 3.算法，完全可以独立于容器和迭代器之外自行发展，只要设计时一迭代器为对外接口就行

### 3.7 SGI_STL 的私房菜：_type_traits

STL只对迭代器加以规范，指定出**iterator_traits**。SGI把这种技法进一步扩大到迭代器以外的世界，于是有了**_type_traits**
...
- 3.是否具备“有用的赋值操作运算符”（non-trivial assignment constructor）
- 4.是否具备“有用的析构函数”（non-trivial destructor）
- 5.是否为POD

如果被萃取的类型，前4种特性都是无用的，在对这个类别进行构造，拷贝，赋值，析构时，就可以采用最有效的措施（如根本不调用那些“无用的”函数），而采用内存直接处理，如malloc()，memcopy()等等，以获得最高效率
...
[<type_traits.h>中的特例化](STL源码剖析/type_traits.h中的特例化.md)
...
[__type_traits例子---uninitialized_fill_n()](STL源码剖析/__type_traits例子.md)
...
- 1.vector维护的就是一个连续线性空间
- 2.vector所需知道的操作如`*、->、++、--、+、-、+=、-=`，普通指针都具备
