...
- - https://www.bilibili.com/video/av15683671?from=search&seid=14170984308276291420)
...
- 表示：
  - 邻接矩阵（适合稠密图）
    ...
    typedef int EdgeType; //边上权值的数据类型，若不带权值，则为0或1
    
    typedef struct {
        VexType Vex[MaxVnum];             //一维数组存储顶点信息
        EdgeType Edge[MaxVnum][MaxVnum];  //邻接矩阵存储边信息
