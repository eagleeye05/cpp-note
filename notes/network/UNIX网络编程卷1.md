...
![img](../../pics/unp-thread-1-1586544468444.png)
...
![img](../../pics/unp-thread-2-1586544468445.png)
...
![img](../../pics/unp-thread-3-1586544468445.png)
...
![img](../../pics/unp-thread-4-1586544468445.png)
...
![img](../../pics/unp-thread-5-1586544468445.png)
...
![img](../../pics/unp-thread-14-1586544468445.png)
...
![img](../../pics/unp-thread-15-1586544468445.png)
...
![img](../../pics/unp-thread-16-1586544468445.png)
...
![img](../../pics/unp-thread-7-1586544468446.png)
...
![img](../../pics/unp-thread-8-1586544468446.png)
...
![img](../../pics/unp-thread-9-1586544468446.png)
...
![img](../../pics/unp-thread-10-1586544468446.png)
...
![img](../../pics/apue-threadctr-21-1586544468446.png)
...
| **TCP回射服务器**                                            | v1                                                           | [客户端](https://github.com/arkingc/unpv13e/blob/master/tcpcliserv/tcpcli01.c) | [str_cli函数](https://github.com/arkingc/unpv13e/blob/master/lib/str_cli.c)(阻塞于标准输入时无法处理来自服务器子进程的FIN分节) |
| ------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
| [服务器](https://github.com/arkingc/unpv13e/blob/master/tcpcliserv/tcpserv01.c)(多进程) | [服务器会产生僵尸子进程](#1客户端正常终止)                   |                                                              |                                                              |
| v2                                                           | [服务器](https://github.com/arkingc/unpv13e/blob/master/tcpcliserv/tcpserv02.c)(多进程) | [处理服务器僵尸子进程，会中断服务器系统调用](#11-使用wait版sig_chld函数处理子进程sigchld信号) |                                                              |
| v3                                                           | [服务器](https://github.com/arkingc/unpv13e/blob/master/tcpcliserv/tcpserv03.c)(多进程) | 处理服务器被中断的系统调用，无法同时处理多个SIGCHLD信号      |                                                              |
| v4                                                           | [客户端](https://github.com/arkingc/unpv13e/blob/master/tcpcliserv/tcpcli04.c) | [正常终止时引起服务器5个子进程终止](#12-使用waitpid版sig_chld函数处理子进程sigchld信号) |                                                              |
| [服务器](https://github.com/arkingc/unpv13e/blob/master/tcpcliserv/tcpserv04.c)(多进程) | [同时处理多个SIGCHLD信号](#12-使用waitpid版sig_chld函数处理子进程sigchld信号) |                                                              |                                                              |
| select                                                       | [客户端](https://github.com/arkingc/unpv13e/blob/master/select/tcpcli01.c) | [str_cli函数](https://github.com/arkingc/unpv13e/blob/master/select/strcliselect01.c)(解决v1版的问题，但无法处理重定向、无法处理I/O缓冲) |                                                              |
| [客户端](https://github.com/arkingc/unpv13e/blob/master/select/tcpcli02.c) | [str_cli函数](https://github.com/arkingc/unpv13e/blob/master/select/strcliselect02.c)(解决前一版的问题) |                                                              |                                                              |
| [服务器](#13-使用select实现tcp回射服务器)(单进程)            | 重写v4版服务器，使用单进程减少了多进程的开销                 |                                                              |                                                              |
| poll                                                         | [服务器](https://github.com/arkingc/unpv13e/blob/master/tcpcliserv/tcpservpoll01.c)(单进程) | 重写v4版服务器，使用单进程减少了多进程的开销                 |                                                              |



<tr>    <td align="center"> **总结** </td>    <td colspan="3" align="center"> **v1-v4：正确处理服务器终止的子进程** </td> </tr> </table>
...
ctrl+d键入EOF，客户端fget返回空指针，str_cli函数返回，在客户端main函数中继续执行str_cli之后的指令，通过exit退出
...
> 问题：服务器父进程accept系统调用被子进程的SIGCHLD信号处理函数中断，返回EINTR错误，父进程无法处理，导致父进程终止   慢系统调用：可能永久阻塞的系统调用。当阻塞于某个慢系统调用的一个进程捕获某个信号且相应信号处理函数返回时，该系统调用可能返回一个EINTR错误。有些内核自动重启某些被中断的系统调用。不过为了便于移植，当我们编写捕获信号的程序时（多数并发服务器捕获SIGCHLD），我们必须对慢系统调用返回EINTR有所准备
...
| **UDP回射服务器**                                            | v1                                                           | [客户端](https://github.com/arkingc/unpv13e/blob/master/udpcliserv/udpcli01.c) | [dg_cli函数](https://github.com/arkingc/unpv13e/blob/master/lib/dg_cli.c)(v1) 问题一：任何进程可向客户端发数据，会与服务器的回射数据报混杂 问题二：客户数据报或服务器应答丢失会使客户永久阻塞于recvfrom [问题三：服务器未启动会使客户端阻塞于recvfrom](#2服务器进程未运行) |
| ------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
| [服务器](https://github.com/arkingc/unpv13e/blob/master/udpcliserv/udpserv01.c)(单进程) | [dg_echo函数](https://github.com/arkingc/unpv13e/blob/master/lib/dg_echo.c) |                                                              |                                                              |
| v2                                                           | [客户端](https://github.com/arkingc/unpv13e/blob/master/udpcliserv/udpcli02.c) | [dg_cli函数](https://github.com/arkingc/unpv13e/blob/master/udpcliserv/dgcliaddr.c)(v2) [处理问题一，验证接收到的响应。但无法处理服务器多宿(多个IP)的情况](#11-验证收到的数据) |                                                              |
| v3                                                           | [客户端](https://github.com/arkingc/unpv13e/blob/master/udpcliserv/udpcli04.c)(connect版) | [dg_cli函数](https://github.com/arkingc/unpv13e/blob/master/udpcliserv/dgcliconnect.c)(v3) |                                                              |
| v4                                                           | [客户端](https://github.com/arkingc/unpv13e/blob/master/udpcliserv/udpcli06.c) | [dg_cli函数](https://github.com/arkingc/unpv13e/blob/master/udpcliserv/dgcliloop1.c)(v4)：写2000个1400字节的UDP数据报给服务器 问题：UDP缺乏流量控制，服务器接收速率慢时，缓冲区被发送端淹没 |                                                              |
| [服务器](https://github.com/arkingc/unpv13e/blob/master/udpcliserv/udpserv06.c)(单进程) | [dg_echo函数](https://github.com/arkingc/unpv13e/blob/master/udpcliserv/dgecholoop1.c) |                                                              |                                                              |
| v5                                                           | [服务器](https://github.com/arkingc/unpv13e/blob/master/udpcliserv/udpserv07.c)(单进程) | [dg_echo函数](https://github.com/arkingc/unpv13e/blob/master/udpcliserv/dgecholoop2.c)(增大接收缓冲区的大小) |                                                              |
| **总结**                                                     | **v1：问题二、三的根本原因是UDP数据传输不可靠**（可使用[套接字超时](#1套接字超时)来处理） **v4-v5：UDP缺乏流量控制，服务器接收的数据报数目不定，依赖诸多因素** |                                                              |                                                              |
...
| 整形     | <sys/types.h>    | int8_t | 带符号的8位整数 |
| -------- | ---------------- | ------ | --------------- |
| uint8_t  | 无符号的8位整数  |        |                 |
| int16_t  | 带符号的16位整数 |        |                 |
| uint16_t | 无符号的16位整数 |        |                 |
...
<tr>     <td rowspan="2"> 时间 </td>     <td rowspan="1"> &lt;sys/time.h&gt; </td>     <td> timeval </td>     <td> 时间结构，包含“秒”和“微妙”成员 </td> </tr> <tr>     <td rowspan="1"> &lt;time.h&gt; </td>     <td> timespec </td>     <td> 时间结构，包含“秒”和“纳秒”成员 </td> </tr>

<tr>     <td rowspan="8"> 套接字地址结构相关 </td>    <td rowspan="3"> &lt;sys/socket.h&gt; </td>     <td> sockaddr </td>    <td> 通用套接字地址结构 </td> </tr> <tr>    <td> sa_family_t </td>    <td> 套接字地址结构的地址族 </td> </tr> <tr>     <td> socklen_t </td>     <td> 套接字地址结构的长度，一般为uint32_t  </td> </tr> <tr>     <td rowspan="5"> &lt;netinet/in.h&gt; </td>    <td> sockaddr_in </td>     <td> IPv4套接字地址结构 </td>  </tr> <tr>     <td> sockaddr_in6 </td>     <td> IPv6套接字地址结构 </td>    </tr> <tr>     <td> sockaddr_storage </td>     <td> 新版通用套接字地址结构 </td> </tr> <tr>     <td> in_addr_t </td>    <td> IPv4地址，一般为uint32_t </td> </tr> <tr>     <td> in_port_t </td>     <td> TCP或UDP端口，一般为uint16_t </td> </tr> </table>
...
<tr>     <td rowspan="4"> 字节序转换 </td>     <td rowspan="4"> &lt;netinet/in.h&gt; </td>     <td> htons </td> </tr> <tr>     <td> htonl </td> </tr> <tr>     <td> ntohs </td> </tr> <tr>     <td> ntohl </td> </tr>

<tr>     <td rowspan="5"> 地址转换 </td>     <td rowspan="5"> &lt;arpa/inet.h&gt; </td>     <td> inet_aton </td> </tr> <tr>     <td> inet_addr </td> </tr> <tr>     <td> inet_ntoa </td> </tr> <tr>     <td> inet_pton </td> </tr> <tr>     <td> inet_ntop </td> </tr>

<tr>     <td rowspan="10"> 基本套接字编程 </td>     <td rowspan="9"> &lt;sys/socket.h&gt; </td>     <td> socket </td> </tr> <tr>     <td> connect </td> </tr> <tr>     <td> bind </td> </tr> <tr>     <td> listen </td> </tr> <tr>     <td> accept </td> </tr> <tr>     <td> recvfrom </td> </tr> <tr>     <td> sendto </td> </tr> <tr>     <td> getsockname </td>     <td> 根据套接字获取本地协议地址 </td> </tr> <tr>     <td> getpeername </td>     <td> 根据套接字获取外地协议地址 </td> </tr> <tr>     <td rowspan="1"> &lt;unistd.h&gt; </td>     <td> close </td> </tr>

<tr>     <td rowspan="6"> I/O复用 </td>     <td rowspan="2"> &lt;sys/select.h&gt; </td>     <td> select </td> </tr> <tr>     <td> pselect </td> </tr> <tr>     <td rowspan="1"> &lt;sys/poll.h&gt; </td>     <td> poll </td> </tr> <tr>     <td rowspan="3"> &lt;sys/epoll.h&gt; </td>     <td> epoll_create </td> </tr> <tr>     <td> epoll_ctl </td> </tr> <tr>     <td> epoll_wait </td> </tr>

<tr>     <td rowspan="6"> 名字与数值转换 </td>     <td rowspan="6"> &lt;netdb.h&gt; </td>     <td> gethostbyname </td>     <td> 主机名字转IP地址(只支持IPv4) </td> </tr> <tr>     <td> gethostbyaddr </td>     <td> IP地址转主机名字(只支持IPv4) </td> </tr> <tr>     <td> getservbyname </td>     <td> 服务名字转端口号 </td> </tr> <tr>     <td> getservbyport </td>     <td> 端口号转服务名字 </td> </tr> <tr>     <td> getaddrinfo </td>     <td> 主机与服务名字转IP地址与端口号 </td> </tr> <tr>     <td> getnameinfo </td>     <td> IP地址与端口号转主机与服务名字 </td> </tr>

<tr>     <td rowspan="6"> 高级I/O函数 </td>     <td rowspan="4"> &lt;sys/socket.h&gt; </td>     <td> recv </td> </tr> <tr>     <td> send </td> </tr> <tr>     <td> recvmsg </td> </tr> <tr>     <td> sendmsg </td> </tr> <tr>     <td rowspan="2"> &lt;sys/uio.h&gt; </td>     <td> readv </td> </tr> <tr>     <td> writev </td> </tr>

<tr>    <td rowspan="1"> **Unix域套接字** </td>    <td rowspan="1"> &lt;sys/socket.h&gt; </td>    <td> [socketpair](#1socketpair函数) </td> </tr>

<tr>     <td rowspan="15"> 线程 </td>     <td rowspan="15"> &lt;pthread.h&gt; </td>     <td> pthread_create </td>     <td> 线程的基本操作：创建线程 </td> </tr> <tr>     <td> pthread_join </td>     <td> 线程的基本操作：等待指定的线程终止 </td> </tr> <tr>     <td> pthread_self </td>     <td> 线程的基本操作：线程获取自身ID </td> </tr> <tr>     <td> pthread_detach </td>     <td> 线程的基本操作：将线程转变为脱离状态 </td> </tr> <tr>     <td> pthread_exit </td>     <td> 线程的基本操作：线程显示终止 </td> </tr> <tr>     <td> pthread_once </td>     <td> 用于线程特定数据 </td> </tr> <tr>     <td> pthread_key_create </td>     <td> 用于线程特定数据 </td> </tr> <tr>     <td> pthread_getspecific </td>     <td> 用于线程特定数据 </td> </tr> <tr>     <td> pthread_setspecific </td>     <td> 用于线程特定数据 </td> </tr> <tr>     <td> pthread_mutex_lock </td>     <td> 互斥锁 </td> </tr> <tr>     <td> pthread_mutex_unlock </td>     <td> 互斥锁 </td> </tr> <tr>     <td> pthread_cond_wait </td>     <td> 条件变量 </td> </tr> <tr>     <td> pthread_cond_signal </td>     <td> 条件变量 </td> </tr> <tr>     <td> pthread_cond_broadcast </td>     <td> 条件变量 </td> </tr> <tr>     <td> pthread_cond_timedwait </td>     <td> 条件变量 </td> </tr> </table>
